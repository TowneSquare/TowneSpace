...
composable_name = 'Cool Sloth'

def generate_image_metadata(folder_path, mirror_folder_path, composable_name):
    ...
    for filename in os.listdir(os.path.join(folder_path, 'Body')):
        if filename.endswith(".png"):
            ...
            new_image_name = f"{composable_name} #{index}" 
            image_metadata = {
                "name": new_image_name,
                "description": "",
                "image": f"{index}.png",
                "edition": 1,
                "attributes": [
                    {"type": folder_name, "value": composable_name},
                    # {"trait_type": "Body", "value": ""},  # Commented out for now
            ...
            image_names_csv.append(f"{composable_name} #")  # Add image name to CSV list
...
metadata, csv_data = generate_image_metadata(folder_path, mirror_folder_path, composable_name)