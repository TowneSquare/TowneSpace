...
export class Studio {
  ...
  async getAllCollections(
    ...
    let indexerResponse = await this.queryIndexer<CollectionsQueryResponse>(
      COLLECTIONS_QUERY,
      variables
    );
   ...
   * @param accountAddress
   */
  async getAllTokens(accountAddress: MaybeHexString): Promise<TokensResponse> {
    const variables = {
      accountAddress: HexString.ensure(accountAddress).hex(),
    };
    let indexerResponse = await this.queryIndexer<TokensQueryResponse>(
      TOKENS_QUERY,
      variables
    );
