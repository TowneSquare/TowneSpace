...
import { COLLECTION_CREATED, COMPOSABLE_TOKEN, COMPOSABLE_TOKEN_CREATED, COMPOSABLE_TOKEN_TESTNET, DA_CREATED, TRAIT_EQUIPPED, TRAIT_TOKEN_CREATED, TRAIT_UNEQUIPPED } from "../constants";
  ...
  export class Events {
    ...
    async getComposableTokenCreatedEvents(): Promise<GetEventsResponse> {
      const composableTokenCreatedEvent = await this.aptos.getModuleEventsByEventType({
    ...
    async getDigitalAssetCreatedEvents(): Promise<GetEventsResponse> {
      const digitalAssetCreatedEvent = await this.aptos.getModuleEventsByEventType({
