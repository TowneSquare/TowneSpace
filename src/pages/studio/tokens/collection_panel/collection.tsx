...
const Collection: React.FC<Props> = ({ data }) => {
   ...
   useEffect(() => {
    ...
    return (
        <div
            className={`px-2 py-2 flex items-center gap-2 rounded-md hover:bg-gray-dark-1 cursor-pointer ${data?.collection_id === "temp" ? "bg-gray-dark-2" : ""}`}
            onClick= {() => onChooseCollection(data)}
        >
            <img src={collectionImage} className="w-[40px]" alt={data.collection_name} />
