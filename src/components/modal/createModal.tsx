...
const CreateModal = () => {
    ...
    const getImageSrc = () => {
        switch (currentIndex) {
            case 1:
                return "/create/game-background.svg";
            default:
    ...
    return (
        <div className={`${isOpen ? "block" : "hidden"} fixed z-[100] inset-0 flex justify-center items-center bg-[#00000050]`}>
            <div className="w-full xl:w-[1023px] mx-4 h-fit flex  bg-gray-dark-1 rounded-[10px]">
                <div className="flex flex-col md:w-[373px] bg-gray-dark-2 px-2 md:px-6 py-10 rounded-l-[10px]">
                    ...
                    <div className="mt-20 flex flex-col w-full">
                        {Menus.map((menu, index) => (
                            <div className={`h-[88px] px-2 md:px-4 flex items-center text-sm md:text-base hover:text-primary-dark-1 whitespace-nowrap ${currentIndex == index ? "bg-gray-dark-1 text-primary-dark-1" : ""} rounded-md cursor-pointer`}
                                key={index}
                ...
                <div className="w-[650px] flex flex-col relative">
                    <div onClick={() => dispatch(toggleCreateModal(false))} className="absolute right-4 top-4 flex items-center justify-center w-10 h-10 rounded-full bg-white bg-opacity-20 cursor-pointer">
                        <CancelIcon />
                    </div>
                    <img src={getImageSrc()} className="bg-[#121212] rounded-tr-[10px]" alt="background" />
                    <div className="h-full px-6 py-6 flex flex-col gap-4 justify-end">
                        ...
                        <div className="w-full flex items-center gap-14 justify-end">
                            <Link to="/">
