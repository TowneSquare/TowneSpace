         ...
         {connected ?
            <div className="flex flex-col gap-2 cursor-pointer">
               <div className="flex items-center gap-2">
                  ...
                  }} className="font-bold">{account?.address.slice(0, 5)}...{account?.address.slice(-3)}
                     <span className="text-xs">&nbsp;&nbsp;&nbsp;{isOpenDropDown ? "▲" : "▼"}</span>
                  </span>
               </div>
               {isOpenDropDown &&
                  <>
                     <div className="flex top-20 bg-gray-dark-2 h-[60px] rounded-b-[10px] w-[200px] items-center justify-center absolute z-50 right-2" onClick={() => disconnect()}>
                        <p className="text-cente hover:font-bold">
                           Disconnect Wallet
                        </p>
                     </div>
                  </>
